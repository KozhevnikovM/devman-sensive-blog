...
def index(request):
    ...
    post_ids = [post.id for post in most_fresh_posts]
    posts_with_comments = Post.objects.filter(id__in=post_ids).annotate(Count('comments', distinct=True))
    id_to_comments = dict(posts_with_comments.values_list('id', 'comments'))
